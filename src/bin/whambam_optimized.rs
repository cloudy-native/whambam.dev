...
mod optimized {
    ...
    impl WorkerPool {
        pub async fn new(config: &TestConfig) -> Result<Self> {
            ...
            Ok(WorkerPool {
                client,
                job_sender,
                metrics_receiver: mpsc::channel::<whambam::tester::Message>(config.concurrent * 2).1,
        ...
        async fn execute_request(
            ...
            // Add some debugging prints for the first few requests
            ...
            let debug_this_request = count < 5 || count % 20 == 0;
            if debug_this_request {
                eprintln!("[Worker] Processing request #{}: {}", count, url);
            }
            let bytes_sent = {
                let mut total = 0u64;
            ...
            let request_result = match result {
                ...
                Err(e) => {
                    if debug_this_request {
                        eprintln!("[Worker] Error on request #{}: {:?}", count, e);
                    }
                    RequestMetric {
                    timestamp: start_time.elapsed().as_fractional_secs(),
                    latency_ms: duration.as_fractional_millis(),
            ...
            if debug_this_request {
                eprintln!("[Worker] Completed request #{}: status={}, latency={}ms", 
                          count, request_result.status_code, request_result.latency_ms);
            }
        ...
        pub fn take_metrics_receiver(&mut self) -> mpsc::Receiver<whambam::tester::Message> {
            ...
            std::mem::replace(&mut self.metrics_receiver, mpsc::channel::<whambam::tester::Message>(1).1)
        }
    ...
    impl TestRunner {
        ...
        pub async fn start(&mut self) -> Result<()> {
            ...
            let _load_test_handle = tokio::spawn(async move {
                ...
                // We no longer need to use the original metrics receiver/sender
                ...
                tokio::spawn(async move {
                    eprintln!("Started metrics forwarding task");
                    let mut count = 0;
                    
                    // We'll now hook into each HTTP request to send metrics
                    while let Some(message) = worker_metrics_rx.recv().await {
                        // Forward to the main metrics processing channel
                        if let Err(e) = tx.send(message).await {
                            eprintln!("Failed to forward metric: {}", e);
                            break;
                        }
                        
                        count += 1;
                        if count % 10 == 0 {
                            eprintln!("Forwarded {} metrics", count);
                        }
                    }
                    
                    // Send completion message when done if not already sent
                    let _ = tx.send(Message::TestComplete).await;
                    eprintln!("Metrics forwarding task completed, forwarded {} metrics", count);
                });
                ...
                eprintln!("Starting worker pool to process up to {} requests with {} concurrent workers", max_requests, config.concurrent);
                ...
                eprintln!("Submitted all jobs: {}", _submitted_jobs);
                worker_pool.stop();
                eprintln!("Worker pool stopped");
            ...
            let _metrics_handle = tokio::spawn(async move {
                eprintln!("Metrics collection task started");
                ...
                while let Some(message) = rx.recv().await {
                    match message {
                        Message::RequestComplete(metric) => {
                            ...
                            metric_count += 1;
                            if metric_count % 10 == 0 || metric_count < 5 {
                                eprintln!("Processed {} metrics", metric_count);
                            }
                        }
                        Message::TestComplete => {
                            eprintln!("Test complete, received {} metrics total", metric_count);
                            break;
                        }
                    }
                }
    ...
    pub fn print_hey_format_report<W: io::Write>(out: &mut W, test_state: &TestState) -> Result<()> {
