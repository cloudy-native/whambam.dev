...
use whambam::tester::{
    HttpMethod, OptimizedRunner, TestConfig, optimized_print_final_report,
};

#[derive(Parser, Clone, Debug)]
#[command(author, version, about = "HTTP load testing tool with lock-free metrics")]
struct Args {
    /// The URL to test
    #[arg(required = true)]
    url: String,
...
async fn main() -> Result<()> {
    ...
    while !is_complete {
        ...
        is_complete = metrics.metrics.is_complete() || metrics.metrics.elapsed_seconds() >= duration_secs as f64 + 2.0;
        if !is_complete {
