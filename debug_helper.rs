...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("URL: {}", url);
    println!("Requests: {}, Concurrent: {}", total_requests, concurrent_requests);
    ...
    let debug_handle = tokio::spawn(async move {
        ...
        loop {
            ...
            print!("[{:.1}s] Sent: {}, Success: {}, Errors: {}, Status codes: {:?}", 
                ...
                match result {
                    Ok(response) => {
                        ...
                        *stats_guard.status_counts.entry(status).or_insert(0) += 1;
                        ...
                        let body = response.text().await.unwrap_or_else(|_| "Failed to get body".to_string());
                        if body.len() > 100 {
                            println!("\nResponse {}: Status {}, Body length: {} bytes", i, status, body.len());
                        } else {
